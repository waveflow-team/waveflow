...
`cuda_configure` depends on the following environment variables:
  ...
  * `TF_CUDA_COMPUTE_CAPABILITIES`: The CUDA compute capabilities. Default is
    `3.5,5.2`.
"""
# TODO(pjarosik) cuda modules and configuration should be in another git/http
# repository - keep it consistent with tf conf (or depend on tf's source).
...
def _compute_capabilities(repository_ctx):
  ...
  # TODO(pjarosik) automatically discover host's card compute capabilities
  # as it is done in tf's/configure.py
...
def _get_cuda_config(repository_ctx):
  ...
  # TODO(pjarosik) make cuddn optional
