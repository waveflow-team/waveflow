...
def hilbert(input, dt=1, name=None):
  ...
  with tf.name_scope(name, op_util.resolve_op_name("Hilbert"), [input]):
    ...
    with tf.control_dependencies([
      tf.assert_greater(input_rank,0,data=[input],message="input can not be a scalar")
    ]):
    ...
    with tf.control_dependencies([
      ...
      non_negative_range = tf.cond(
        tf.equal(tf.mod(n, 2), 0),
          ...
          false_fn=lambda: tf.range(start=0, limit=((n-1)//2)+1, dtype=dtype)
      )
      negative_range = tf.cond(
        tf.equal(tf.mod(n, 2), 0),
          true_fn=lambda: tf.range(start=-1*(n//2), limit=0, dtype=dtype),
          false_fn=lambda: tf.range(start=-1*((n-1)//2), limit=0, dtype=dtype)
      )
      return tf.concat([tf.div(non_negative_range, T),tf.div(negative_range, T)], -1)
